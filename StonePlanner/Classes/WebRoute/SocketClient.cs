...
namespace StonePlanner.Classes.WebRoute
{
    internal class SocketClient
    {
        ...
        private byte[] _bufferSize = new byte[1024 * 1024 * 2];
        ...
        public void StartClient()
        {
            try
            {
                ...
                //接收数据
                int length = _socket.Receive(_bufferSize);
                System.Console.WriteLine("接收服务器{0},消息:{1}", _socket.RemoteEndPoint.ToString(), Encoding.UTF8.GetString(_bufferSize, 0, length));
                _buffer = Encoding.UTF8.GetString(_bufferSize, 0, length);
                //向服务器发送消息
                Thread.Sleep(2000);
